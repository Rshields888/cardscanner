...
function buildQueryFromText(text: string) {
  // Very light heuristics: pick likely year, brand, player tokens, set, and card no (#123)
  const lower = text.toLowerCase();
  const yearMatch = lower.match(/\b(19|20)\d{2}\b/);
  const brands = ["topps","panini","prizm","mosaic","optic","select","donruss","bowman","chrome","fleer","upper deck","score","prizim"]; // include common misspellings
  const brand = brands.find(b => lower.includes(b)) || "";
  const numMatch = lower.match(/#\s?([0-9]{1,4}[a-z]?)/) || lower.match(/no\.\s?([0-9]{1,4}[a-z]?)/);

  // crude player extraction: longest Title Case sequence up to 3 words
  const nameMatch = (text.match(/[A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2}/g) || [])
    .sort((a,b)=>b.length - a.length)[0] || "";

  // set keywords: RC, rookie, silver, refractor, psa, bgs, sgc
  const flags = [];
